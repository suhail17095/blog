...
function Blogs(props) {
    function Reload() {
        ...
        axios.post("https://blog-backend-r81t.onrender.com/show_blog", { user_id: user_id }).then((res1) => {
    ...
    function del(e,title,content){
        const user_id=props.UserId.user_id;
        axios.post("https://blog-backend-r81t.onrender.com/delete_blog",{user_id:user_id,title:title,content:content}).then((res)=>
        {
            alert(res.data.msg);
            Reload();
        })
